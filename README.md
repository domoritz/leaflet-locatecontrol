...
|------------|-----------|-------------------|----------|
| `oneshot_callback` | `function`  | function([lat,lng]) to be called *only once* when location is found. if this is set, `stop()` is called automatically when a location is first found. | `null` |
