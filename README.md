...
| `compassThreshold` | `number` or `false` | Minimum compass accuracy in degrees for displaying the compass. iOS uses `webkitCompassAccuracy` when available. Set to `false` to always show the compass when heading data is available. | `25` |
