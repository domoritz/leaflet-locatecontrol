...
| `keepCurrentZoomLevel` | `boolean` or `Array` | Only pan when setting the view. Set to True (for any zoom), or a zoom range like [13,18] which will only zoom the map when outside the range. Keeps the current zoom, when current view within the defined range. | `false` |
